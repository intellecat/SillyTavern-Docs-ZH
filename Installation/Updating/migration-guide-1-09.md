...
**重要!** 在执行任何操作之前,请对您的安装进行*完整备份*。在此过程中您可能会*丢失数据*,因此请不要忽视此警告。

不确定要备份哪些文件?请参阅此处的列表:[如何更新 SillyTavern](/Installation/Updating/index.md#updating-from-1120-to-1120)

### git 安装

1. 在您的 SillyTavern 安装文件夹中打开终端提示符(cmd、PowerShell、Termux 等)。
2. 输入 `git fetch` 然后 `git pull` 来拉取更新。
...
4. 如果没有错误,请跳到下一项。您可能会看到类似以下内容:
   ...
   如果您确实想保存这些更改,请从备份中恢复。

5. 输入 `npm install` 然后 `npm run start` 来测试一切是否正常运行。
6. 尽情享受!如有需要,从备份中恢复您的数据。

### fatal: invalid reference: release

如果您从旧远程(迁移到组织仓库之前)克隆了单个分支,可能会发生这种情况。要解决此问题,您需要从新远程添加并获取分支:
